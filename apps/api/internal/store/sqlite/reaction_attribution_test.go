...
func TestMessagePageAttributesReactions(t *testing.T) {
	...
	st := newTestStore(t)
	owner, err := st.EnsureBootstrap(ctx, "Attribution Owner", "attribution-owner@example.com")
	if err != nil {
		t.Fatal(err)
	}
	...
	member, err := st.CreateUser(ctx, store.CreateUserInput{DisplayName: "Attribution Member", Email: "attribution-member@example.com"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReactionAttributionStaysBounded(t *testing.T) {
	...
	ctx := context.Background()
	st := newTestStore(t)
	owner, err := st.EnsureBootstrap(ctx, "Bounded Owner", "bounded-owner@example.com")
	if err != nil {
		t.Fatal(err)
	}
	workspaces, err := st.ListWorkspaces(ctx, owner.ID)
	...
	for i := range total {
		reactor, err := st.CreateUser(ctx, store.CreateUserInput{
			...
			Email:       fmt.Sprintf("bounded-reactor-%02d@example.com", i),
		})
		if err != nil {
			t.Fatal(err)
		}
