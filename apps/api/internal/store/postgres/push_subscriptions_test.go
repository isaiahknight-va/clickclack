...
func TestPushSubscriptionsCascadeWithTheUser(t *testing.T) {
	...
	if _, err := st.db.ExecContext(ctx, `DELETE FROM users WHERE id = $1`, member.ID); err != nil {
		t.Fatal(err)
	}
	var remaining int
	if err := st.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM user_push_subscriptions WHERE user_id = $1`, member.ID).Scan(&remaining); err != nil {
		t.Fatal(err)
	}
	if remaining != 0 {
...
func newMigratedPostgresTestStore(t *testing.T) *Store {
	t.Helper()
